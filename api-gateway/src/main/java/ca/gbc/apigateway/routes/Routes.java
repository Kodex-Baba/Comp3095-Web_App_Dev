        ...
        return GatewayRouterFunctions.route("product_service")
                .route(RequestPredicates.path("/api/product"), request -> {
                    ...
                    try {
                        ...
                        {
                            log.error("Error occurred while handling request: {}", e.getMessage(), e);
                            return ServerResponse.status(500).body("An error occurred routing product request");
                        }
                    }
                })
                .build();
        ...
        return GatewayRouterFunctions.route("order_service")
                .route(RequestPredicates.path("/api/order"), request -> {
                    ...
                    try {
                        ...
                        {
                            log.error("Error occurred while handling request: {}", e.getMessage(), e);
                            return ServerResponse.status(500).body("An error occurred routing order request");
                        }
                    }
                })
                .build();
        ...
        return GatewayRouterFunctions.route("inventory_service")
                .route(RequestPredicates.path("/api/inventory"), request -> {
                    ...
                    try {
                        ...
                        {
                            log.error("Error occurred while handling request: {}", e.getMessage(), e);
                            return ServerResponse.status(500).body("An error occurred routing inventory request");
                        }
                    }
                })
                .build();
